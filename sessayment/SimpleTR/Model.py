...
class Model:
    ...
    def setupCNN(self):
        ...
        for i in range(numLayers):
            ...
            pool = tf.nn.max_pool(
                relu, (1, poolVals[i][0], poolVals[i][1], 1), (1, strideVals[i][0], strideVals[i][1], 1), "VALID"
            )
    ...
    def setupCTC(self):
        ...
        self.loss = tf.reduce_mean(
            tf.nn.ctc_loss(
                labels=self.gtTexts, inputs=self.ctcIn3dTBC, sequence_length=self.seqLen, ctc_merge_repeated=True
            )
        )
        ...
        self.lossPerElement = tf.nn.ctc_loss(
            labels=self.gtTexts, inputs=self.savedCtcInput, sequence_length=self.seqLen, ctc_merge_repeated=True
        )
        ...
        if self.decoderType == DecoderType.BestPath:
            ...
            self.decoder = tf.nn.ctc_beam_search_decoder(
                inputs=self.ctcIn3dTBC, sequence_length=self.seqLen, beam_width=50, merge_repeated=False
            )
            ...
            wordChars = open(os.path.join(self.PROJECT_ROOT, "model/wordCharList.txt")).read().splitlines()[0]
            corpus = open(os.path.join(self.PROJECT_ROOT, "data/corpus.txt")).read()
    ...
    def setupTF(self):
        ...
        modelDir = os.path.join(self.PROJECT_ROOT, "model/")
    ...
    def trainBatch(self, batch):
        ...
        rate = (
            0.01 if self.batchesTrained < 10 else (0.001 if self.batchesTrained < 10000 else 0.0001)
        )  # decay learning rate
    ...
    def dumpNNOutput(self, rnnOutput):
        ...
        dumpDir = os.path.join(self.PROJECT_ROOT, "dump/")
    ...
    def save(self):
        "save model to file"
        self.snapID += 1
        self.saver.save(self.sess, os.path.join(self.PROJECT_ROOT, "model/snapshot"), global_step=self.snapID)
